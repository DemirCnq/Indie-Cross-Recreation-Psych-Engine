...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
        ...
        if (SONG.song.toLowerCase() == 'technicolor-tussle')//to run only on this song
        {
			cupshid = new FlxSprite();
			cupshid.frames = Paths.getSparrowAtlas('stages/cup/HUD-effect/CUpheqdshid');
			cupshid.animation.addByPrefix('cupshid', 'Cupheadshit_gif instance', 24, false);
        ...
        if (SONG.song.toLowerCase() == 'knockout')//to run only on this song
        {
			rain = new FlxSprite();
			rain.frames = Paths.getSparrowAtlas('stages/cup/HUD-effect/Rain');
			rain.animation.addByPrefix('rain', 'Rain', 24, false);
			rain.screenCenter();
			add(rain);
			rain.cameras = [camHUD];
        }
